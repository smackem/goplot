...
func Test_lex(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Keywords",
			src:  "let log sqrt sin cos tan asin acos atan",
			want: []token{
				token{Type: ttLet, Lexeme: "let"},
		...
		{
			name: "Expression",
			src:  "let f(x) = sin x * cos((x) ** PI/4.0)",
			want: []token{
				token{Type: ttLet, Lexeme: "let"},
				token{Type: ttIdent, Lexeme: "f"},
				token{Type: ttLParen, Lexeme: "("},
				token{Type: ttIdent, Lexeme: "x"},
				token{Type: ttRParen, Lexeme: ")"},
				token{Type: ttEq, Lexeme: "="},
